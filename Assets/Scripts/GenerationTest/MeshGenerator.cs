...
public static class MeshGenerator
{
    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, MeshSettings meshSettings, int levelOfDetail)
    {
        ...
        for (int y = 0; y < numVerticesPerLine; y++)
        {
            for(int x = 0; x < numVerticesPerLine; x++) // Create vertex
            {
                bool isSkippedVertex = x > 2 && x < numVerticesPerLine - 3 && y > 2 && y < numVerticesPerLine - 3 && ((x - 2) % skipIncrement != 0 || (y - 2) % skipIncrement != 0);
                ...
                {
                    bool isOutOfMeshVertex = y == 0 || y == numVerticesPerLine - 1 || x == 0 || x == numVerticesPerLine - 1;
                    bool isEdgeMeshVertex = (y == 1 || y == numVerticesPerLine - 2 || x == 1 || x == numVerticesPerLine - 2) && !isOutOfMeshVertex;
                    bool isMainVertex = (x - 2) % skipIncrement == 0 && (y - 2) % skipIncrement == 0 && !isOutOfMeshVertex && !isEdgeMeshVertex;
                    bool isEdgeConnectionVertex = (y == 2 || y == numVerticesPerLine - 3 || x == 2 || x == numVerticesPerLine - 3) && !isOutOfMeshVertex && !isEdgeMeshVertex && !isMainVertex;
...
public class MeshData
{
    ...
    public MeshData(int numVerticesPerLine, int skipIncrement, bool useFlatShading)
    {
        ...
        int numTotalMainVertices = numMainVerticesPerLine * numMainVerticesPerLine;

        vertices = new Vector3[numMeshEdgeVertices + numEdgeConnectionVertices + numTotalMainVertices];
    ...
    public void AddVertex(Vector3 vertexPosition, Vector2 uv, int vertexIndex)
    {
        ...
        else
        {
            ...
            uvs[vertexIndex] = new Vector2(0, vertexPosition.y); // This is currently unused and textures are using world position instead of uv wrapping
        }
    }
